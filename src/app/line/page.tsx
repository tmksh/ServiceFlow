      ...
      <div className="space-y-4 lg:space-y-5 animate-fade-in">
        ...
        <div className="grid grid-cols-3 gap-3 lg:gap-4">
          {[
            ...
            <Card key={i} className={cn("p-3 lg:p-5", s.bg)}>
              <div className="flex flex-col lg:flex-row items-center lg:items-center gap-2 lg:gap-3 text-center lg:text-left">
                <div className={cn("p-2 lg:p-2.5 rounded-xl", s.color)}><s.icon size={18} className="text-white" /></div>
                <div>
                  <p className="text-lg lg:text-2xl font-bold text-slate-900">{s.v}</p>
                  <p className="text-[10px] lg:text-sm text-slate-500 leading-tight">{s.l}</p>
                </div>
              </div>
            </Card>
          ))}
        ...
        <Card className="p-3 lg:p-4">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              <div className="p-2 bg-indigo-100 rounded-xl shrink-0"><Shield size={18} className="text-indigo-600" /></div>
              <div className="min-w-0">
                <h3 className="font-semibold text-slate-800 text-sm">自動案件登録モード</h3>
                <p className="text-xs text-slate-400 hidden sm:block">LINEメッセージを自動解析し案件として即時登録</p>
              </div>
            </div>
            <button onClick={() => setAutoMode(!autoMode)} className={cn("relative w-14 h-7 rounded-full transition-colors shrink-0", autoMode ? "bg-indigo-600" : "bg-slate-300")}>
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-slate-800">{m.group}</span>
                    <Badge className={stBadge[m.status]}>{stLabel[m.status]}</Badge>
                  </div>
        ...
        {/* Mobile: Bottom Sheet for parsed preview */}
